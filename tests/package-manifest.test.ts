...
describe("package manifest compatibility", () => {
  ...
  it("pins the OpenCode V2 plugin API and its compatible OpenTUI runtime", () => {
    expect(pkg.peerDependencies?.["@opencode-ai/plugin"]).toBe("0.0.0-beta-18286");
    expect(pkg.devDependencies?.["@opencode-ai/plugin"]).toBe("0.0.0-beta-18286");
  ...
  it("keeps the TypeScript 7 toolchain explicit without suppressing the known peer mismatch", () => {
    ...
    expect(typescriptValidator).toContain('const EXPECTED_PLUGIN_VERSION = "0.0.0-beta-18286";');
